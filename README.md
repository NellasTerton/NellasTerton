...
- **[Rehearsio](https://github.com/NellasTerton/rehearsio)** — тренажёр голосовых собеседований: по тексту вакансии AI-интервьюер звонит и ведёт живой разговор голосом (Groq + OpenAI TTS), а не отвечает по шаблону — задаёт уточняющие вопросы на расплывчатые ответы, сам завершает звонок и выдаёт письменный разбор с оценкой по каждому вопросу. Полный биллинг-цикл: Auth.js, Neon Postgres, Stripe-подписка с идемпотентным вебхуком, rate limiting.
...
### ⚙️ Продукты и автоматизация

*Сервисы, которые постоянно сверяются с внешним источником: расписание, синхронизация, пересчёт состояния.*

- **[UEFA Predict 26/27](https://github.com/NellasTerton/uefa-predictions)** — PWA для футбольных прогнозов среди друзей: Лига чемпионов, Лига Европы и Лига конференций в одном приложении, 396 матчей сезона. Общий рейтинг с разбивкой очков по каждому турниру. Результаты приезжают сами: Edge Function на Deno раз в 10 минут по расписанию pg_cron сверяет матчи с ESPN по идентификаторам, пересчитывает очки за прогнозы и пересобирает таблицу лидеров. Supabase с Row Level Security, безвозвратное удаление аккаунта через служебную функцию, жёсткая блокировка прогнозов по времени стартового свистка. В репозитории две версии: актуальная клубная и архивная под чемпионат мира.
